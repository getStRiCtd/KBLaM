...
# Copied from transformers.models.gemma.modeling_gemma.GemmaRotaryEmbedding with gemma->Qwen2, Gemma->Qwen2
...
class Qwen2LongRoPEScaledRotaryEmbedding(Qwen2RotaryEmbedding):
...
class KBLaMQwen2Attention(nn.Module):
    ...
    def _init_rope(self):
        if self.rope_scaling is None:
            ...
            if scaling_type == "longrope":
                self.rotary_emb = Qwen2LongRoPEScaledRotaryEmbedding(
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = KBLaMQwen2Attention(config=config, layer_idx=layer_idx)
...
class KBLaMQwen2ForCausalLM(Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    # Copied from transformers.models.llama.modeling_llama.LlamaForCausalLM.__init__ with Llama->Qwen2
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        >>> model = Qwen2ForCausalLM.from_pretrained("microsoft/phi-3-mini-4k-instruct")
...
@add_start_docstrings(
    """
    The [`Qwen2Model`] with a sequence classification head on top (linear layer).
...
# Copied from transformers.models.llama.modeling_llama.LlamaForSequenceClassification with Llama->Qwen2, LLAMA->Qwen2, self.transformer->self.model, transformer_outputs->model_outputs
...
@add_start_docstrings(
    """
    [`Qwen2Model`] with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
    Named-Entity-Recognition (NER) tasks.
    """,
    Qwen2_START_DOCSTRING,
)
# Copied from transformers.models.mpt.modeling_mpt.MptForTokenClassification with Mpt->Qwen2,MPT->Qwen2,self.transformer->self.model,transformer_outputs->model_outputs
